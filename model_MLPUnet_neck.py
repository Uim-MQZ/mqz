...
class MLPMixerUnet_neck(tf.keras.models.Model):
  def __init__(self, S, C, num_of_mlp_blocks, image_size, batch_size):
    super(MLPMixerUnet_neck, self).__init__()
  ...
  def residual_unet(self,images):
      ...
      mlp_mixer_neck = self.mlp_mixer_neck(pool4)

      up1 = UpSampling2D(size=(2, 2))(mlp_mixer_neck)
